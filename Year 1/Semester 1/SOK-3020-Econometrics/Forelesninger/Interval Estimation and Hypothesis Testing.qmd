...
- If $-2.024 \leq t \leq 2.024$, we will not reject the null hypothesis
>>>>>>> 9293bb493c7f615c37a0aa45f86b48e15d57a575
...
- Reject  $H_0$ when the $p$-value is less than, or equal to, the level of significance $\alpha$. 
...
=======
- The null hypothesis is $H0: β2 ≤ 5.5$ vs $H1: β2 > 5.5$

- The value of the test statistic is: $t= \frac{b2-c}{se(b2)} = \frac{10.21-5.5}{2.09}= 2.25$

- The p-value is: $p = P[t_{(38)}\ge 2.25]=1-P[t_{(38)}\leq 2.25] = 1-0.9848=0.0152$

- If $\alpha=0.01$, we do not reject the $H_0$ 
>>>>>>> 9293bb493c7f615c37a0aa45f86b48e15d57a575
...
=======
- We donot  reject $H_0$ for $\alpha=0.05, 0.10$ or $\alpha=0.20$ b/c $p > \alpha$.
>>>>>>> 9293bb493c7f615c37a0aa45f86b48e15d57a575
...
=======
 The null hypothesis is $H0: β2 = 0$ vs $H1: β2  \neq 0$

- The value of the test statistic is: $t= 4.88$
>>>>>>> 9293bb493c7f615c37a0aa45f86b48e15d57a575
...
=======
- As an example of a linear combination, if we let  $c_1 = 1$ and $c_2 = x_0$, then we have
>>>>>>> 9293bb493c7f615c37a0aa45f86b48e15d57a575
...
=======
The estimator $\hat \lambda$ is unbiased of $\lambda$.

---

The variance of $\hat \lambda$ is

$$\text{var} (\hat \lambda | x) = \text{var}(c_1b_1 + c_2b_2) $$
$$= c_1^2 \text{var}(b_1) + c_2^2 \text{var}(b_2) + 2c_1c_2 \text{cov}(b_1, b_2)$$

We estimate $\hat \lambda$  by replacing the unknown variances and covariances with their estimated variances and covariances $b1$ and $b2$.



$$ se(\hat \lambda) = \sqrt var(\hat \lambda|x)$$
$$= \sqrt (\hat{\text{var}}(c_1b_1 + c_2b_2) $$
---

- If in addition SR6 holds, or if the sample is large, the least squares estimators b1 and b2 have normal distributions

- It is also true that linear combinations of normally distributed variables are normally distributed, so that

  $$ 
  \hat \lambda | x = c_1b_1 + c_2b_2 \sim N(\lambda, \text{var}(\hat \lambda | x))
  $$
---
...
=======
The t-statistic is: 
$$t = \frac{(c_1\beta_1 + c_2\beta_2) - c_0}{\text{se}(c_1\beta_1 + c_2\beta_2)} \sim t_{(N-2)}$$
...
```{r,echo=TRUE}
# Example 3.9: Test linear combination
# H0: b1 + 20*b2 <= 250 vs H1: b1 + 20*b2 > 250
library(car)
dmt <- deltaMethod(fit, "b1 + 20*b2 - 250", parameterNames = c("b1", "b2"))
t_val <- dmt$Estimate / dmt$SE
t_val > qt(0.95, 38)  # TRUE -> reject H0
```
