...
For discrete random variable $X$, the pdf $f(x)$ gives: $$f(x) = P(X = x)$$
...
$$E(X) = \mu_X = \sum_{all\ x} x \cdot f(x)$$ It's a **weighted average** where probabilities are the weights.
...
**Variance** measures the spread of a distribution: $$var(X) = \sigma_X^2 = E[(X - \mu_X)^2] = E(X^2) - \mu_X^2$$
...
Number of successes in $n$ independent trials: $$P(X = k) = \binom{n}{k}p^k(1-p)^{n-k}$$ - $E(X) = np$, $var(X) = np(1-p)$
...
This powerful result means: - The overall expectation equals the expectation of conditional expectations - Very useful in econometric models
...
This says total variance equals: - Variance of conditional means + Average of conditional variances - "Between-group" variation + "Within-group" variation
...
**Inversion Method**: If $F(x)$ is invertible, 1. Generate $U \sim Uniform(0,1)$\
2. Set $U = F(Y)$\
3. Solve: $Y = F^{-1}(U)$
